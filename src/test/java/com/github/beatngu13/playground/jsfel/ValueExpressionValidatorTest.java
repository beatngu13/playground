...
class ValueExpressionValidatorTest {
	...
	void test_valid() throws Exception {
		...
		assertThatCode(() -> ValueExpressionValidator.validate(xhtml, helloBean)).doesNotThrowAnyException();
	}

	@Test
	void test_invalid() throws Exception {
		Path xhtml = Paths.get(getClass().getResource("/invalid.xhtml").toURI());
		assertThatCode(() -> ValueExpressionValidator.validate(xhtml, helloBean)).isExactlyInstanceOf(AssertionError.class);
	}

	@Test
	void test_nested_valid() throws Exception {
		Path xhtml = Paths.get(getClass().getResource("/nested-valid.xhtml").toURI());
		assertThatCode(() -> ValueExpressionValidator.validate(xhtml, nestedBean)).doesNotThrowAnyException();
	}

	@Test
	void test_nested_invalid() throws Exception {
		Path xhtml = Paths.get(getClass().getResource("/nested-invalid.xhtml").toURI());
		assertThatCode(() -> ValueExpressionValidator.validate(xhtml, nestedBean)).isExactlyInstanceOf(AssertionError.class);
	}
